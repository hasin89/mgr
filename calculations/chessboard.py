...
#         self.find_subcorners(harrisdst, corners, scene.view)
    ...
    def getPoints(self,corners2Shifted,gray):
        ...
        gray = gamma
        ret, binary = cv2.threshold(gray,120,1,cv2.THRESH_BINARY_INV)
        size = 5
        ...
        print binary.shape
        print finalChessboarMap.shape
        binary = cv2.bitwise_and(binary,finalChessboarMap)
        
#         binary = cv2.adaptiveThreshold(gray,1,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,31,1)
